...
async function findByPlateAndStatus(plate: string, status: string = "inside") {
  const result = (
    await database.sql(
      "SELECT * FROM portaria.vehicle WHERE plate = $1 AND status = $2",
      [plate, status],
    )
  ).rows[0];
  return result;
...
async function deletePlateAndStatus(plate: string, status: string = "inside") {
  return await database.sql(
    "DELETE FROM portaria.vehicle WHERE plate = $1 AND status = $2",
    [plate, status],
  );
}
